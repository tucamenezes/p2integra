...
public class ModeloController extends DaoInterfaceImplements<Modelo> implements DaoInterface<Modelo> {

	public ModeloController(Class<Modelo> persistenceClass) {
	...
	public String listarModelos(@PathVariable("status") String status) throws Exception {
		...
		return new Gson().toJson(HibernateUtilHQL.getListSqlHQL("from Modelo as a where a.status='" + status + "'"));
